    ...
    try:
        ...
        try:
            with open(JOBS_FILE, 'r', encoding='utf-8') as f:
                ...
                jobs = data.get("jobs", [])
                if jobs:
                    # Auto-repair: rewrite with proper escaping.  Wrap the
                    # save in jobs_transaction so a corrupt-file recovery
                    # in one process can't race with another process's RMW.
                    # (Nested inside any outer jobs_transaction via the
                    # thread-local counter — safe to enter twice.)
